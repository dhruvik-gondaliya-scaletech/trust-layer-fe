        ...
        <div className="flex-1 overflow-y-auto px-0.5 space-y-5 scrollbar-none pb-28">
          ...
          <FormField control={form.control} name="title" rules={{ required: "Item title is required" }} render={({ field }) => (
            <Field className="flex flex-col gap-1.5 border-none p-0">
              <FieldLabel htmlFor="item-title" className="text-sm font-semibold text-foreground/85">
                Title
              </FieldLabel>
              <FormControl>
                <Input
                  ...
                  className="rounded-2xl h-12 px-4 border text-base font-semibold bg-background placeholder:text-muted-foreground/50 placeholder:font-normal"
                  {...field}
                />
              </FormControl>
          ...
          <FormField control={form.control} name="price" rules={{
            ...
            <Field className="flex flex-col gap-1.5 border-none p-0">
              <FieldLabel htmlFor="item-price" className="text-sm font-semibold text-foreground/85">
                Price (USD)
              </FieldLabel>
          ...
          <div className="grid grid-cols-[1.3fr_1fr] gap-3">
            <FormField control={form.control} name="category" rules={{ required: "Category is required" }} render={({ field }) => (
              <Field className="flex flex-col gap-1.5 border-none p-0">
                <FieldLabel htmlFor="item-category" className="text-sm font-semibold text-foreground/85">
                  Category
                </FieldLabel>
                <FormControl>
                  <Input
              ...
              {isGraded && (
                ...
                >
                  <FormField control={form.control} name="gradedSerial" rules={{ required: isGraded ? "Serial number is required" : false }} render={({ field }) => (
                    <Field className="flex flex-col gap-1.5 border-none p-0">
                      <FieldLabel htmlFor="item-serial" className="text-sm font-semibold text-foreground/85">
                        Serial Number
                      </FieldLabel>
                      <FormControl>
                        <Input
          ...
          <FormField control={form.control} name="description" render={({ field }) => (
            <Field className="flex flex-col gap-1.5 border-none p-0">
              <FieldLabel htmlFor="item-desc" className="text-sm font-semibold text-foreground/85">
                Description
              </FieldLabel>
              <FormControl>
                <Textarea
                  ...
                  rows={3}
                  className="border-border/80 resize-none rounded-2xl p-4 text-base font-semibold bg-background placeholder:text-muted-foreground/50 placeholder:font-normal"
                  {...field}
                />
              </FormControl>
            </Field>
